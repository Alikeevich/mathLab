...
export function VsScreen({ player, opponentName, opponentMMR, onComplete }: Props) {
  ...
  useEffect(() => {
    // Простая таймлайн-логика
    const t1 = setTimeout(() => setStage('idle'), 100); // Почти сразу показываем
    const t2 = setTimeout(() => setStage('exit'), 3500); // Уходим
    const t3 = setTimeout(onComplete, 4000); // Завершаем
  ...
  // Фейковый винрейт соперника для красоты
  const oWinRate = Math.min(95, Math.max(40, 50 + Math.random() * 20));
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-slate-950 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 md:gap-16 w-full max-w-4xl justify-center">
        ...
        {/* VS Бейдж (Простая анимация масштаба) */}
        <div 
          className={`
            shrink-0 w-20 h-20 bg-white rounded-full flex items-center justify-center
            shadow-[0_0_40px_rgba(255,255,255,0.3)] z-20 md:absolute md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2
        ...
        <PlayerCard 
          ...
          rank={oRank} 
          winRate={oWinRate}
          skin="electric" // Боту даем скин для красоты
          stage={stage}
        />

...
const EQ_STEPS: EqToken[][] = [
  [{ id: 'int', tex: '\\int_0^{\\pi}' }, { id: 'integrand', tex: '\\sin^2(x)' }, { id: 'dx', tex: '\\,dx' }],
  [{ id: 'int', tex: '\\int_0^{\\pi}' }, { id: 'integrand', tex: '\\dfrac{1-\\cos 2x}{2}' }, { id: 'dx', tex: '\\,dx' }],
  [{ id: 'left', tex: '\\dfrac{1}{2}\\!\\int_0^{\\pi}\\!dx' }, { id: 'minus', tex: '-' }, { id: 'right', tex: '\\dfrac{1}{2}\\!\\int_0^{\\pi}\\!\\cos 2x\\,dx' }],
  ...
  return (
    <motion.div key="act3" exit={{ opacity: 0 }} className="absolute inset-0 bg-[#020617] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative w-[320px] max-w-[88vw] flex flex-col z-20">
        <motion.div
          className="absolute inset-0 rounded-[2rem] bg-slate-900/40 backdrop-blur-2xl border border-white/10 shadow-[0_30px_100px_rgba(0,0,0,0.9)]"
          animate={{ opacity: dissolving ? 0 : 1 }}
          transition={{ duration: 0.35 }}
        />

        {/* Header */}
        <motion.div className="relative bg-white/5 border-b border-white/10 rounded-t-[2rem] pt-10 pb-4 px-6 flex justify-between items-center" animate={{ opacity: dissolving ? 0 : 1 }}>
        ...
        {/* Equation Zone */}
        <motion.div className="relative flex-1 flex flex-col items-center justify-center py-10 px-4 min-h-[190px]" animate={{ scale: dissolving ? 1.28 : 1 }}>
          <motion.div className="absolute inset-3 rounded-2xl border" animate={{ backgroundColor: isAnswer ? 'rgba(16,185,129,0.08)' : dissolving ? 'rgba(0,0,0,0)' : 'rgba(255,255,255,0.04)', borderColor: isAnswer ? 'rgba(16,185,129,0.35)' : dissolving ? 'rgba(0,0,0,0)' : 'rgba(255,255,255,0.08)' }} />
          <div className="relative h-5 mb-3">
            <AnimatePresence mode="wait">
              {isSolving && stepIdx > 0 && (
                <motion.span key={stepIdx} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} className={`font-mono text-[10px] uppercase tracking-[0.3em] ${isAnswer ? 'text-emerald-400' : 'text-slate-500'}`}>
                  {STEP_LABELS[stepIdx]}
                </motion.span>
              )}
            </AnimatePresence>
          ...
          <div className="relative flex flex-wrap items-center justify-center gap-x-1 gap-y-1 px-2">
            <AnimatePresence mode="popLayout">
              {tokens.map(token => (
                <motion.div key={`${token.id}:${token.tex}`} layout initial={{ opacity: 0, y: 20, filter: 'blur(8px)' }} animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }} exit={{ opacity: 0, y: -20, filter: 'blur(8px)' }} className={`${isAnswer ? 'text-emerald-300' : 'text-white'} ${token.big ? 'text-5xl' : 'text-xl md:text-2xl'}`}>
        ...
        <motion.div animate={{ opacity: dissolving ? 0 : 1 }}>
      ...
      <AnimatePresence>
        {isAnswer && (
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }} className="absolute bottom-28 flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-400 drop-shadow-[0_0_8px_rgba(16,185,129,1)]" />
            <motion.span animate={{ letterSpacing: '0.4em' }} className="text-sm font-black text-emerald-400 uppercase">Solved</motion.span>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
...
const Act4_PremiumFinale = ({ onAction, onClose }: { onAction: () => void; onClose: () => void }) => {
  return (
    <motion.div key="act4" className="absolute inset-0 bg-[#010308] flex flex-col items-center justify-center z-[200] overflow-hidden">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.3 }} transition={{ duration: 3 }} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80vw] h-[80vw] bg-blue-900/30 blur-[120px] rounded-full pointer-events-none" />
      <div className="relative z-40 flex flex-col items-center justify-center w-full px-4 text-center">
        <motion.h1 initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 3, duration: 1.5 }} className="text-6xl md:text-[8rem] font-black text-white uppercase tracking-tighter leading-none">
          MATHLAB <br/> <span className="text-cyan-400 text-3xl md:text-5xl tracking-[0.4em]">PVP</span>
        </motion.h1>
        <motion.button
          initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 4 }}
          onClick={() => { onClose(); onAction(); }}
          className="mt-16 px-10 py-4 bg-white text-black font-bold uppercase tracking-widest flex items-center gap-3 rounded-lg shadow-[0_10px_40px_rgba(255,255,255,0.2)]"
        >
    ...
    <div className="fixed inset-0 z-[9999] bg-black overflow-hidden font-sans select-none epic-color-grade">
      ...
      <div className="absolute top-0 left-0 h-1 bg-slate-900 w-full z-[10000]">
        <motion.div
          ...
          initial={{ width: '0%' }}
          animate={active ? { width: '100%' } : { width: '0%' }} // ← CHANGED

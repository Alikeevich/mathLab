...
import { 
  ...
  Loader, Mail, User, School, Briefcase, AlertTriangle, XCircle 
} from 'lucide-react';
...
export function BecomeTeacherModal({ onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Создаем запись в таблице teacher_requests со всеми новыми полями
      ...
      <div className="fixed inset-0 z-[250] bg-slate-900/95 backdrop-blur-sm flex items-center justify-center p-4 animate-in zoom-in duration-300">
        <div className="bg-slate-800 border border-emerald-500/50 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl relative overflow-hidden">
          ...
          <p className="text-slate-400 mb-6 text-sm leading-relaxed">
            Уважаемый(ая) <strong>{fullName}</strong>, мы проверим ваши документы в течение 24 часов и отправим ответ на почту: <br/>
            <span className="text-cyan-400 font-mono font-bold mt-1 inline-block">{contactEmail}</span>
          </p>
          <button 
            onClick={onClose} 
    ...
    <div className="fixed inset-0 z-[200] bg-slate-900/95 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-cyan-500/30 w-full max-w-lg rounded-3xl shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-800/50 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1 font-medium uppercase tracking-wide">Верификация личности</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar">
          {/* Инфо блок */}
          <div className="mb-6 p-4 bg-cyan-900/10 border border-cyan-500/20 rounded-xl text-sm text-cyan-100/90 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-4 opacity-10">
              <FileText className="w-24 h-24 text-cyan-400" />
            </div>
            <h4 className="font-bold mb-2 flex items-center gap-2 text-cyan-400 relative z-10">
              <FileText className="w-4 h-4"/> Возможности ментора:
            </h4>
            <ul className="list-disc pl-5 space-y-1 text-xs md:text-sm relative z-10">
              <li>Создание закрытых турниров</li>
              <li>Доступ к Генератору задач</li>
              <li>Специальный статус в профиле</li>
            </ul>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* ФИО */}
            <div>
              <label className="block text-slate-400 text-[10px] font-bold uppercase tracking-wider mb-1.5 ml-1">ФИО полностью</label>
              <div className="relative group">
                ...
                <input 
                  ...
                  placeholder="Напр: Асанов Арман Бахытович"
                  className="w-full pl-10 pr-4 py-3 bg-slate-800 border border-slate-600 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500 transition-all"
                  required
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {/* Должность */}
              <div>
                <label className="block text-slate-400 text-[10px] font-bold uppercase tracking-wider mb-1.5 ml-1">Должность</label>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              ...
              {/* Школа */}
              <div>
                <label className="block text-slate-400 text-[10px] font-bold uppercase tracking-wider mb-1.5 ml-1">Школа / ВУЗ</label>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            ...
            {/* Email */}
            <div>
              <label className="block text-slate-400 text-[10px] font-bold uppercase tracking-wider mb-1.5 ml-1">Email для связи</label>
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            ...
            {/* Загрузка файла */}
            <div>
              <label className="block text-slate-400 text-[10px] font-bold uppercase tracking-wider mb-1.5 ml-1">Документ (Диплом / Справка)</label>
              <div className="relative group cursor-pointer">
                ...
                />
                <div className={`border-2 border-dashed rounded-xl p-6 text-center transition-all duration-300 ${file ? 'border-emerald-500 bg-emerald-500/5' : 'border-slate-700 group-hover:border-cyan-400 group-hover:bg-slate-800'}`}>
                  {file ? (
                    <div className="flex flex-col items-center gap-2">
                      <CheckCircle className="w-8 h-8 text-emerald-400 animate-in zoom-in" />
                      <span className="text-emerald-400 font-bold text-sm truncate max-w-[240px]">{file.name}</span>
                    </div>
                  ) : (
                    <div className="text-slate-500 group-hover:text-cyan-400 transition-colors flex flex-col items-center">
                      ...
                      <span className="text-[10px] opacity-60 mt-1">PDF, JPG, PNG (до 5 МБ)</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700/50">
              <label className="flex items-start gap-3 cursor-pointer group">
                ...
                />
                <span className="text-[10px] md:text-xs text-slate-400 group-hover:text-slate-300 transition-colors leading-snug">
                  Я подтверждаю подлинность предоставленных данных. Я понимаю, что подделка документов приведет к вечной блокировке аккаунта.
                </span>
              </label>
            </div>
            ...
            <button 
              type="submit" 
              disabled={!file || !agreed || !fullName || loading}
              className="w-full py-4 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 disabled:from-slate-800 disabled:to-slate-800 disabled:text-slate-600 disabled:cursor-not-allowed text-white font-bold rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 active:scale-95"
            >

...
export function TournamentLobby({ tournamentId }: LobbyProps) {
  ...
  const [loading, setLoading] = useState(false); // Добавили стейт
  ...
  async function checkForActiveDuel() {
    if (!user) return;
    const { data } = await supabase.from('duels').select('id').eq('tournament_id', tournamentId).eq('status', 'active').or(`player1_id.eq.${user.id},player2_id.eq.${user.id}`).maybeSingle();
    setActiveDuelId(data ? data.id : null);
  }

  if (activeDuelId) {
    return <TournamentPlay duelId={activeDuelId} onFinished={() => { setActiveDuelId(null); fetchParticipants(); }} />;
  }
    ...
    return (
      <div className="h-full p-4 md:p-8 flex flex-col">
        {!activeDuelId && status === 'active' && (
           <div className="bg-slate-800 p-4 text-center text-slate-400 mb-4 rounded-xl border border-slate-700 animate-pulse">Ожидайте завершения других матчей... Ваш бой скоро появится.</div>
        )}
        <div className="flex-1 overflow-hidden"><TournamentBracket tournamentId={tournamentId} onEnterMatch={() => {}} /></div>
      </div>
    );
  }
    ...
    <div className="flex items-center justify-center h-full p-4">
      <div className="w-full max-w-5xl bg-slate-900/90 border border-cyan-500/30 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <div className="flex items-center gap-3 mb-1"><Shield className="w-8 h-8 text-cyan-400" /><h1 className="text-3xl font-black text-white uppercase italic">Турнирное Лобби</h1></div>
          ...
          <div className="flex items-center gap-3 bg-slate-800 px-6 py-3 rounded-full border border-slate-700">
            <Loader className="w-5 h-5 text-cyan-400 animate-spin" /><span className="text-white font-medium animate-pulse">Ожидание организатора...</span>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {participants.map((p) => (
              <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl border bg-slate-900 border-slate-700">
                <div className="w-10 h-10 bg-gradient-to-br from-slate-700 to-slate-800 rounded-full flex items-center justify-center text-white font-bold text-sm">{p.profiles?.username?.[0]?.toUpperCase() || '?'}</div>
